...
(function (factory) {
    ...
    if (xhrobj.upload) {
      xhrobj.upload.addEventListener(
        ...
        function (event) {
          ...
          file.widget.settings.onUploadProgress.call(
            file.widget.element,
            file.id,
            percent
          );
  ...
  DmUploaderFile.prototype.cancel = function (abort) {
    ...
    if (
      myStatus === FileStatus.UPLOADING ||
      (abort && myStatus === FileStatus.PENDING)
    ) {
  ...
  DmUploaderFile.prototype.canUpload = function () {
    return (
      this.status === FileStatus.PENDING || this.status === FileStatus.FAILED
    );
  };
  ...
  DmUploader.prototype.init = function () {
    ...
    var input = widget.element.is("input[type=file]")
      ? widget.element
      : widget.element.find("input[type=file]");
  ...
  DmUploader.prototype.initDnD = function () {
    ...
    $(document)
      .off("drop." + pluginName)
      .on("drop." + pluginName, function (evt) {
        evt.preventDefault();
    ...
    $(document)
      .off("dragenter." + pluginName)
      .on("dragenter." + pluginName, function (evt) {
        evt.preventDefault();
    ...
    $(document)
      .off("dragleave." + pluginName)
      .on("dragleave." + pluginName, function (evt) {
        evt.preventDefault();
    ...
    $(document)
      .off("dragover." + pluginName)
      .on("dragover." + pluginName, function (evt) {
        evt.preventDefault();
      });
  };
  ...
  DmUploader.prototype.addFiles = function (files) {
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      var can_continue = this.settings.onNewFile.call(
        this.element,
        file.id,
        file.data
      );
      ...
      this.each(function () {
        ...
        if (plugin instanceof DmUploader) {
          if (typeof plugin.methods[options] === "function") {
            plugin.methods[options].apply(
              plugin,
              Array.prototype.slice.call(args, 1)
            );
          } else {
            $.error(
              "Method " + options + " does not exist in jQuery.dmUploader"
            );
          }
        } else {
